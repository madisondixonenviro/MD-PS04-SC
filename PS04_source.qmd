...
-   The relationship in figure 1 looks positive and reasonably linear.
:::
...
-   It is not reasonable to conclude an actual causation relationship between the variables poverty and trump_support, correlation between data variables does not equal causation.
:::
...
-   **`r y_hat`** is the percentage of Trump support expected at 6% white poverty.
:::
...
-   I don't think predicting at a value of 85% white poverty would be a good idea as the regression line fitted only covers up to ~20% white poverty, predicting out of the range would not be a good idea as the data would be unreliable.
:::
...
```{r}
# Type your code and comments inside the code chunk
summarize(trump, cor(non_white, trump_support))
```
:::
...
newt <- get_regression_table(nw_mod)


```

-   The y-intercept is `r newt[2,2]`.

-   The slope is `r newt[1,2]`.
:::
...
```{r}
# Type your code and comments inside the code chunk
pred = newt[1,2]*.70+newt[2,2]
```

-   The predicted Trump support for a region with a population of 70% non-white people is `r pred`%.
:::
...
-   The variable non_white covers a larger range of percentages but the poverty variable has a higher correlation coefficient (when using absolute value), so poverty is the better explanatory variable.
:::
...
```{r}
# Type your code and comments inside the code chunk
pred2 = newt[1,2]*.61+newt[2,2]
```

**A.** The predicted Trump support for a region with a population of 61% non-white people is `r pred2`%.
...
**B.** An overestimate by 2% as `r act[1,3]` is an actual support value for an area of 61% non-white people from the data set.
